...
#This section was for timing purpose. Not needed for the project.
...
#with Image.open('flowers/train/1/image_06734.jpg') as im:
...
def predict(image_path, model_path, topk=5):
    ...
    with torch.no_grad():
        ...
        top_class_indexes =             top_class.detach().cpu().numpy().astype(np.int).squeeze().tolist()
        
        #print(model.class_to_idx)
        top_class_classes = []
        for index in top_class_indexes:
            #print(index)
            key_class = next(key_class for                 key_class, idx in model.class_to_idx.items() if idx == index)
            #print(key_class)
...
# Great project by the way. It was a bit tough at first but I managed to push it through
